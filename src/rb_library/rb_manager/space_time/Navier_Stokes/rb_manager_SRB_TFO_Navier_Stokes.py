...
class RbManagerSRB_TFO_NavierStokes(rbmstNS.RbManagerSpaceTimeNavierStokes,
    ...
    def build_rb_affine_decompositions(self, operators=None):
        ...
        # rbmsrbtfoS.RbManagerSRB_TFO_Stokes.build_rb_affine_decompositions(self, operators=operators)
    ...
    def build_rb_parametric_RHS(self, param):
        ...
        """
        raise NotImplementedError
        # return rbmsrbtfoS.RbManagerSRB_TFO_Stokes.build_rb_parametric_RHS(self, param)
    ...
    def build_reduced_convective_term(self, x):
        ...
        nl_term = np.zeros(self.M_N_space['velocity'])
        for l in range(N_comp):
            for m in range(N_comp):
                nl_term += u[l] * u[m] * affine_components[l, m]
        nl_term *= self.M_bdf_rhs * dt

        res = np.hstack([nl_term, np.zeros(self.M_N_space['pressure']), np.zeros(np.sum(self.M_N_space['lambda']).astype(int))])
    ...
    def build_reduced_convective_jacobian(self, x):
        ...
        nl_jac = np.zeros((self.M_N_space['velocity'], self.M_N_space['velocity']))
        for l in range(N_comp):
            nl_jac += u[l] * affine_components[l]
        nl_jac *= self.M_bdf_rhs * dt
